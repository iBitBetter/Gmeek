...
class GMEEK():
    ...
    def _clean_description(self, text):
        """清洗 meta description：去除 HTML 标签 / markdown 语法 / 残留配置块 / 多余空白，并安全截断。"""
        if not text:
            return ""
        import re
    ...
    def addOnePostJson(self,issue):
        if len(issue.labels)>=1:
            ...
            if issue.body==None:
                ...
                desc_body = self._clean_description(clean_body.split(period)[0])
                self.blogBase[listJsonName][postNum]["description"] = desc_body
            ...
            # 改造：支持 issue 末尾 <!-- ##{...}## --> 里的 "description" 自定义描述
            if "description" in postConfig and postConfig["description"]:
                self.blogBase[listJsonName][postNum]["description"]=str(postConfig["description"]).replace("\"", "\'")
